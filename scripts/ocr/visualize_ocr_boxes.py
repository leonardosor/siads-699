...
def visualize_image_with_boxes(
    image_data, yolo_regions=None, tesseract_words=None, label=None, save_path=None
):
    """
...
def visualize_from_parquet(parquet_path, image_index=0, save_output=True):
    ...
    processor = OCRProcessor(use_yolo_ocr=True, use_tesseract=True, save_to_db=False)
    ...
    if save_output:
        ...
        save_path = (
            output_dir
            / f"ocr_visualization_{Path(parquet_path).stem}_img{image_index}.png"
        )
...
def main():
    ...
    visualize_from_parquet(
        parquet_path=parquet_file, image_index=image_index, save_output=True
    )
