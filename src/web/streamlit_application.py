...
def _extract_text_from_bbox(image: Image.Image, bbox: Tuple[float, float, float, float]) -> dict:
    """
...
def _format_detections(result, image: Image.Image = None, extract_text: bool = False) -> pd.DataFrame:
    ...
    if boxes is None or len(boxes) == 0:
        return pd.DataFrame(columns=["label", "confidence", "x1", "y1", "x2", "y2", "extracted_text", "ocr_confidence"])
...
def _run_inference(image: Image.Image, confidence: float, iou: float, extract_text: bool = True) -> Tuple[Image.Image, pd.DataFrame]:
...
def main() -> None:
    ...
    st.caption("Upload a JPG/PNG/PDF to generate Michigan-branded bounding boxes from your YOLOv8 model.")
    ...
    with st.sidebar:
        ...
        extract_text = st.checkbox("Extract text from detected regions", value=True,
    ...
    uploads = st.file_uploader(
        "Upload one or more JPG/PNG/PDF files", type=ALLOWED_TYPES, accept_multiple_files=True
    )

    if not uploads:
        st.info("Waiting for uploads… drag a JPG/PNG/PDF into the widget above to begin.")
        return
    ...
    for uploaded in uploads:
        ...
        for page_num, raw_image in enumerate(images, start=1):
            ...
            st.image(raw_image, caption=f"Original{page_suffix}", use_container_width=True)
            spinner_text = f"Running YOLOv8 inference and OCR{page_suffix}…" if extract_text else f"Running YOLOv8 inference{page_suffix}…"
            with st.spinner(spinner_text):
                annotated, detections = _run_inference(raw_image, confidence, iou, extract_text=extract_text)

            if detections.empty:
                st.warning(f"No bounding boxes detected with the current thresholds{page_suffix}.")
                continue

            annotated_buffer = io.BytesIO()
            annotated.save(annotated_buffer, format="JPEG")

            st.image(annotated_buffer.getvalue(), caption=f"UM-Branded Bounding Boxes{page_suffix}", use_container_width=True)

            download_filename = f"{Path(uploaded.name).stem}_page{page_num}_umich_bboxes.jpg" if len(images) > 1 else f"{Path(uploaded.name).stem}_umich_bboxes.jpg"
            st.download_button(
            ...
            pretty["confidence"] = (pretty["confidence"] * 100).round(1).astype(str) + "%"

            # If OCR was performed, format those columns too
            if extract_text and "ocr_confidence" in pretty.columns:
                pretty["ocr_confidence"] = pretty["ocr_confidence"].round(1).astype(str) + "%"
                # Reorder columns to show extracted text prominently
                display_cols = ["label", "extracted_text", "confidence", "ocr_confidence", "word_count", "x1", "y1", "x2", "y2"]
            ...
            if extract_text and "extracted_text" in detections.columns and len(detections) > 0:
                ...
                with col1:
                    ...
                    csv_filename = f"{Path(uploaded.name).stem}_page{page_num}_ocr_results.csv" if len(images) > 1 else f"{Path(uploaded.name).stem}_ocr_results.csv"
                    st.download_button(
                ...
                with col2:
                    ...
                    json_filename = f"{Path(uploaded.name).stem}_page{page_num}_ocr_results.json" if len(images) > 1 else f"{Path(uploaded.name).stem}_ocr_results.json"
                    st.download_button(
            ...
            if extract_text and "extracted_text" in detections.columns:
                ...
                if len(texts_found) > 0:
                    ...
                    for idx, row in texts_found.iterrows():
                        with st.expander(f"📄 {row['label']} (confidence: {row['confidence']:.1%})"):
                            st.write(row["extracted_text"])
                            st.caption(f"OCR Confidence: {row['ocr_confidence']:.1f}% | Words: {row['word_count']}")
                else:
                    st.warning(f"No text extracted from detected regions{page_suffix}. Try adjusting confidence thresholds or check image quality.")
