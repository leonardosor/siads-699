...
OMEGA_INPUT_FILE = REPO_ROOT / "data" / "output" / "omega_value.json"


def calculate_confidence_interval(
    sample_size: int, proportion: float, confidence_level: float = 0.95
):
    """
...
def load_omega_value():
    """
    Load the calculated Omega value from the file generated by calculate_dataset_variability.py.
    Returns a dictionary with omega and related statistics, or None if file not found.
    """
    if not OMEGA_INPUT_FILE.exists():
        return None

    try:
        with open(OMEGA_INPUT_FILE, "r") as f:
            data = json.load(f)
        return data
    except Exception as e:
        print(f"[WARN] Error reading omega file: {e}")
        return None
...
def main():
    ...
    if omega_data and omega_data.get("omega") is not None:
        ...
        print("\n[INFO] No calculated Omega value found.")
        print("       Run 'python notebooks/calculate_dataset_variability.py' to calculate it.")
        measured_omega = 4.5  # Default fallback

    if items_per_image > 0:
        print(f"\nEstimated items (boxes) per image: {items_per_image:.1f}")
        ...
        for acc, acc_desc in scenarios:
            ...
            # Build omega levels list with measured value if available
            omega_levels = []
            if measured_omega is not None:
                omega_levels.append((measured_omega, f"Measured ({measured_variability})"))
            
            # Add reference values for comparison
            omega_levels.extend([
                (10, "High Variability"),
                (50, "Medium Variability"),
                (100, "Low Variability"),
            ])
